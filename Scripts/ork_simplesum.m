...
%                               ORK simple sum
% -------------------------------------------------------------------------
% This function creates a sub-affinity matrix for each couple of frames,
% considering each frame gap in frame_gap_range, and then creates a single
% K matrix by just summing them

function[Kernels] = ork_simplesum(method, seq_list, seq_range, data, max_hypos, frame_gap_range, hypos, ork_method)
    ...
    for s_i = seq_range
        ...
        % if strcmp(method, 'affine') || strcmp(method, 'homography')
        %     frame_gap_range = 1;
        % end
        
        for frame_gap = frame_gap_range
            for f_i = 1:seq_data.nFrames-frame_gap
                Res = [];                
                mdl_idx = find(seq_hypos.r == f_i & seq_hypos.v == f_i+frame_gap)';
                ...
                for h_i = mdl_idx                    
                    ...
                    %% Calculate residual
                    Res = [Res feval(resfn,seq_hypos.H(:,h_i),normalized_data)];           
                end
