function [] = moseg_subset(seq_list, seq_range, data, max_hypos, frame_gap_range, alpha_range, best_alphas, gamma_range, ...
    kernels_A, kernels_H, kernels_F, clustering_method)
 
    for alpha = alpha_range

        alphas = [alpha, alpha, alpha];
        moseg(seq_list, seq_range, data, max_hypos, frame_gap_range, alphas, gamma_range, kernels_A, kernels_H, kernels_F, clustering_method);
    end

    if length(best_alphas) == 3
        moseg(seq_list, seq_range, data, max_hypos, frame_gap_range, best_alphas, gamma_range, kernels_A, kernels_H, kernels_F, clustering_method);
    end
end

function moseg(seq_list, seq_range, data, max_hypos, frame_gap_range, alphas, gamma_range, ...
    kernels_A, kernels_H, kernels_F, clustering_method)
    ...
    for gamma = gamma_range
        ...
        error_filepath = fullfile(result_path,sprintf('Error_RandSamp_nhpf-%d_alphas-%d,%d,%d_gamma-%g.mat',...
            max_hypos,alphas(1),alphas(2),alphas(3),gamma));
        ...
        parfor s_i = seq_range
            ...
            [U_Subset, ~ , ~, ~, red_lap_trace] = func_Subset_eig(K,nMotion,gamma,epsilon,MaxItr);
        ...
        fprintf('alphas = [%d, %d, %d], gamma = %.3f, mean error = %.2f%%\n',alphas(1),alphas(2),alphas(3),gamma,100*mean(error(seq_range)));
