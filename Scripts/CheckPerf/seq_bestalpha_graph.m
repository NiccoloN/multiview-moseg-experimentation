% -------------------------------------------------------------------------
% Generation of sequence-dependent error graphs, using the best alpha for 
% each sequence
% -------------------------------------------------------------------------
...
function seq_bestalpha_graph(Error_matrix, Clusters_matrix, alpha_range, lambda_range, gamma_range, sel_lambda, sel_gamma, sel_seqs, run_parameters)
    ...
    % Calculate the best alpha matrix
    ...
    % Open a new figure
    fig = figure;
    title('Sequence-by-sequence performances with best alpha');
    ...
    Averaged_matrix_seq = zeros(length(sel_seqs) + 2, length(methods) + 2);

    % Plot the graphs
    for method_ind = 1:length(methods)
        best_alpha_ind_method = Best_alpha_ind_matrix(method_ind, :);
        for seq_ind = 1:length(sel_seqs)
            err_best_alpha = Averaged_matrix(1, method_ind, seq_ind, ...
                     best_alpha_ind_method(seq_ind), sel_lambda_ind, sel_gamma_ind);
            Averaged_matrix_seq(seq_ind, method_ind) = err_best_alpha;
        end
    end
    ...
    for seq_ind = 1:length(sel_seqs)
        majvoting_errors = zeros(1, num_runs);
        for run_ind = 1:num_runs
            Clusters = [];
            best_alpha_ind_seq = Best_alpha_ind_matrix(:, seq_ind);
            for method_ind = 1:length(methods)
                Clusters(:, method_ind) = Clusters_matrix{run_ind, method_ind, seq_ind, ...
                    best_alpha_ind_seq(method_ind), sel_lambda_ind, sel_gamma_ind};
            end
    ...
    title(sprintf('Dataset: %s\nORK %s evaluated using %s method\nClustering: %s\nFrame gap: up to %d frames\nGraph made selecting the best alpha for each sequence', run_parameters.sel_dataset, run_parameters.ork_philosophy, run_parameters.ork_method, run_parameters.clustering_method, run_parameters.frame_gap_range))
end
