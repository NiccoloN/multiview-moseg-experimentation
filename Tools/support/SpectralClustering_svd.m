...
function [Grps , SingVals, LapKernel, RedLapTrace] = SpectralClustering_svd(CKSym,n,mth, clustering_method)
...
switch lower(mth)
    case 'unnormalized'
        ...
        RedLapTrace = trace(kerKU.'*LapKU*kerKU);
        LapKernel = kerKU;
    case 'randomwalk'
        ...
        RedLapTrace = trace(kerKN.'*LapKN*kerKN);
        LapKernel = kerKN;
    case 'normalized'        
        ...
        switch clustering_method
            ...
            case 'linkage'
                Grps = cluster(linkage(kerKS), 'maxclust', n);
            otherwise
                error('The chosen clustering method is not valid');
        end

        RedLapTrace = trace(kerKS.'*LapKS*kerKS);
        LapKernel = kerKS;
end
%
